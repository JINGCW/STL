...
void Text::Draw(SDL_Renderer* rR, string sText, int X, int Y, int fontSize) {
...
void Text::Draw(SDL_Renderer* rR, string sText, int X, int Y, int fontSize, int iR, int iG, int iB) {
	this->fontSize = fontSize;
	this->extraLeft = 0;
	this->nextExtraLeft = 0;
...
void Text::DrawCenterX(SDL_Renderer* rR, string sText, int Y, int fontSize, int iR, int iG, int iB) {
	int X = 800 / 2 - getTextWidth(sText, fontSize) / 2;

	Draw(rR, sText, X, Y, fontSize, iR, iG, iB);
}

void Text::Draw(SDL_Renderer* rR, string sText, int X, int Y, int iWidth, int iHeight) {
	for(unsigned int i = 0; i < sText.size(); i++) {
		rCrop.x = getPos(sText.at(i));
...
void Text::DrawWS(SDL_Renderer* rR, string sText, int X, int Y,int iR, int iG, int iB, int fontSize) {
	this->fontSize = fontSize;
	this->extraLeft = 0;
	this->nextExtraLeft = 0;
...
int Text::getTextWidth(string sText, int fontSize) {
...
void Text::setFont(SDL_Renderer* rR, string fileName) {
